...
quarter_on_quarter = (
    pd.read_csv(
        "data/cpi/cpi_quarterly_quarter_on_quarter.csv", parse_dates=["start", "end"]
    )
...
# %%
dat1 = (
    quarter_on_quarter.set_index(["start", "end", "cal_quarter", "group"])
    .unstack(["group"])
    .reset_index()
)
...
# %%
quarters = (
    quarter_on_quarter[["start", "end", "cal_quarter"]]
    .drop_duplicates()
    .sort_values("start")
)
quarters
...
last_crude_oil_price_per_quarter[f"{col}_pct_change"] = (
    ...
    / last_crude_oil_price_per_quarter[col].shift(1)
    * 100
)
